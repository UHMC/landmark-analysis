...
while (True):
    ...
    if (files != ''):  # Only run when folder is not empty
        for x in files:
            ...
            with open(filename + "-odapi_output.json",
                      'wb') as output:  # Dump ODAPI output into /srv/ObjectDB/odapi_output
                output.write(odapi_output)
            os.chdir('/srv/ObjectDB/unprocessed')
            data_image = ('/srv/ObjectDB/processed/' + filename, '/srv/ObjectDB/EXIF/' + filename + '-exif.json',
                          '/srv/ObjectDB/odapi_output/' + filename + '-odapi_output.json')  # Information to be added to the database
